...
;NB-DENNE FILEN HAR EN RAR FEILMELDING VED DEBUGGING
; Konstanter
  cr equ 13 ; Vognretur
...
; Skriver ut sifferet lagret i ecx. Ingen sjekk på verdiområde.
  ...
  push ecx
  push edx
  mov edx, 0    ;rest havner her (remainder)
  mov eax, ecx  ;tall som skal deles havner her (henter fra ecx)
  mov ebx, 10   ;bruker 10 som divisor
  div ebx   ;deler eax med ebx, feks eax = 12: div ebx -> eax = 1, edx = 2
  push edx  ;lagrer remainder for utskriving
  add eax,'0'       ; converter første siffer til ascii.
  mov [siffer],eax
  mov ecx,siffer
  mov edx,1
  mov ebx,STDOUT
  mov eax,SYS_WRITE
  int 80h
  pop ecx       ; henter remainder fra utregningen
  add ecx,'0'       ; converter remainder (andre siffer) til ascii.
  mov [siffer],ecx
  mov ecx,siffer
  mov edx,1
  mov ebx,STDOUT
